...
Future<String> cargarMesociclo(String idClienteSeleccionado) async {
  ...
  guardarEnBBDD(
    ...
    DatabaseReference ref = firebaseDatabase.ref('${getUserID()}/mesociclos');
...
cargandoDatos() async{
DatabaseReference ref = firebaseDatabase.ref(getUserID());
    ref.child('nombreUsuario').onValue.listen((e) async{
    if (e.snapshot.val()!=null)
      nombreUsuario=e.snapshot.val();
   ...
   if (displayName != null && displayName!=''){
    ...
    ref.child('sexo').onValue.listen((e) {
         sexo= e.snapshot.val();
         ref.child('imagen').onValue.listen((e) async{
      String imgUrl2=await getPhoto();
if (e.snapshot.val() != null)
          imgUrl = e.snapshot.val();
        else 
        if (imgUrl2!=null) {
        imgUrl= imgUrl2;
       } else { imgUrl ='No';
